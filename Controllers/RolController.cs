...
namespace controlAcademico_web.Controllers
{
    ...
    {
        private readonly IRolService? _objetoService;

        public RolController(IRolService? objetoService)
        {
        ...
        public async Task<IActionResult> Crear(RolModel guardarModelo)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Editar(RolModel guardarModelo)
        {
            if (ModelState.IsValid)
        ...
        public async Task<string> EditarEliminar(int codigoRegistro)
        {
            ...
            string respuestaModelo = string.Empty;
            RolModel guardarModelo = new RolModel();


            guardarModelo = await _objetoService!.RegistroPorCodigo(codigoRegistro);

            if (guardarModelo.codigoRol > 0)
            {
                //TODO: Quemado mientras se hace lo de la autenticación
                guardarModelo.nombreRol = "eliminado" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
